var ColorUtils = (() => {
  ...
  */
  ...
  var rgbaToHex = (rgba) =>
    `#${_intToPaddedHex(rgba.r)}${_intToPaddedHex(rgba.g)}${_intToPaddedHex(
      rgba.b,
    )}`;
  ...
  var intArrayToHex = (int_array) =>
    `#${_intToPaddedHex(int_array[0])}${_intToPaddedHex(
      int_array[1],
    )}${_intToPaddedHex(int_array[2])}`;
  ...
  var bytes3ToIntArray = (bytes3) => [
    parseInt(bytes3.substr(2, 2), 16),
    parseInt(bytes3.substr(4, 2), 16),
    parseInt(bytes3.substr(6, 2), 16),
    255,
  ];
